...
export const DOMService = {
    ...
    async getAudioBase64(audioId: string, maxWaitMs: number = 15000): Promise<string> {
        ...
        if (!blobUrl && targetRow) {
            ...
            const playBtn = targetRow.querySelector(
                'button[aria-label*="play" i], button[aria-label*="reproducir" i], button[aria-label*="nota de voz" i], button[aria-label*="voice" i], [data-icon*="play"], [data-icon*="ptt"], [data-testid*="play"], [data-testid*="ptt"], [role="button"][aria-label*="reproducir" i], [role="button"][aria-label*="play" i], div._ak27, div._ak28, button'
            ) as HTMLElement | null;
            ...
            if (playBtn) {
                ...
                clickable.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, cancelable: true, clientX, clientY, pointerType: 'mouse' }));
                clickable.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, clientX, clientY, view: window }));
                clickable.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, cancelable: true, clientX, clientY, pointerType: 'mouse' }));
                clickable.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, cancelable: true, clientX, clientY, view: window }));
                clickable.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, clientX, clientY, view: window }));
                clickable.click();
            }

            // Escuchar evento de audio iniciado desde el main world
            const audioPlayingPromise = new Promise<string>((resolve) => {
                const onPlay = (e: any) => {
                    if (e.detail && e.detail.src) {
                        window.removeEventListener('tactica:audio-playing', onPlay);
                        resolve(e.detail.src);
                    }
                };
                window.addEventListener('tactica:audio-playing', onPlay);
                setTimeout(() => {
                    window.removeEventListener('tactica:audio-playing', onPlay);
                }, maxWaitMs);
            });

            // Polling en el DOM en paralelo
            const startTime = Date.now();
            while (Date.now() - startTime < maxWaitMs && !blobUrl) {
            ...
            // Si no se encontró en el polling, esperar el evento de audio con un margen de 2s
            if (!blobUrl) {
                const eventSrc = await Promise.race([
                    audioPlayingPromise,
                    new Promise<string>((resolve) => setTimeout(() => resolve(''), 2000))
                ]);
                if (eventSrc && eventSrc.startsWith('blob:')) {
                    blobUrl = eventSrc;
                }
            }
        ...
        if (!blobUrl || !blobUrl.startsWith('blob:')) {
            throw new Error('No se pudo reproducir automáticamente este audio. Por favor dale Play en WhatsApp Web y volvé a presionar Transcribir.');
        }
