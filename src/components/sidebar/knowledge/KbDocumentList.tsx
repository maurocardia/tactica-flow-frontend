...
import { Trash2, Loader2, FileText, Table, FileSpreadsheet } from 'lucide-react';
...
// Ícono por tipo de archivo, según la extensión del nombre (más confiable que el mimeType, que
// puede venir genérico según el navegador que subió el archivo).
function DocTypeIcon({ filename }: { filename: string }) {
  const ext = filename.toLowerCase().split('.').pop() || '';
  if (ext === 'csv') return <Table className="w-3.5 h-3.5 text-emerald-500 shrink-0" />;
  if (ext === 'xlsx' || ext === 'xls') return <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-600 shrink-0" />;
